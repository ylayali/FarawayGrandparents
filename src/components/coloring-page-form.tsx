...
export function ColoringPageForm({
    ...
    const [burstingInFlags, setBurstingInFlags] = React.useState<boolean[]>([]);
    ...
    // Update individual names array and bursting-in flags when image count changes
    React.useEffect(() => {
        ...
        if (type !== 'straight-copy') {
            ...
            setBurstingInFlags(prev => {
                const newArray = [...prev];
                // Ensure we have exactly as many bursting-in flags as images
                while (newArray.length < currentCount) {
                    newArray.push(false);
                }
                while (newArray.length > currentCount) {
    ...
    const handleBurstingInToggle = (index: number) => {
        setBurstingInFlags(prev => {
            const newArray = [...prev];
    ...
    const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const formData: ColoringPageFormData = {
            ...
            setPiece: setPiece,
            burstingInFlags: burstingInFlags
        };
                        ...
                        <div className='space-y-3'>
                            ...
                            {imageFiles.map((_, index) => (
                                <div key={index} className='space-y-1'>
                                    ...
                                    />
                                    {/* Bursting In checkbox for cartoon portrait mode */}
                                    {type === 'cartoon-portrait' && (
                                        <div className='flex items-center space-x-2 pt-1'>
                                            <input
                                                type='checkbox'
                                                id={`bursting-in-${index}`}
                                                checked={burstingInFlags[index] || false}
                                                onChange={() => handleBurstingInToggle(index)}
                                                disabled={isLoading}
                                                className='h-4 w-4 rounded border-white/40 bg-black text-white focus:ring-white/50'
                                            />
                                            <Label htmlFor={`bursting-in-${index}`} className='cursor-pointer text-sm text-white/80'>
                                                Bursting In
                                            </Label>
                                        </div>
                                    )}
